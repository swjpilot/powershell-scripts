...
Param (
  ...
  [Parameter(Mandatory = $false)][switch]$DisableUsers = $true,
  ...
  Process {
    Try {
      ForEach ($Item in $global:Results){
        ...
        Move-ADObject -Identity $Item.DistinguishedName -TargetPath "OU=zzzDisabled Users No Access,DC=Pomeroy,DC=msft"
