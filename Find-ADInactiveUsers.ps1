...
Param (
  [Parameter(Mandatory = $false)][string][ValidateSet('All', 'OnlyInactiveUsers', 'OnlyServiceAccounts', 'OnlyNeverLoggedOn', 'AllExceptServiceAccounts', 'AllExceptNeverLoggedOn')]$SearchScope = 'AllExceptServiceAccounts',
  [Parameter(Mandatory = $false)][string]$Searchbase = "DC=Pomeroy,DC=msft",
  [Parameter(Mandatory = $false)][int]$DaysInactive = 90,
  [Parameter(Mandatory = $false)][string]$ServiceAccountIdentifier = 'svc',
  [Parameter(Mandatory = $false)][string]$ReportFilePath = 'C:\InactiveReports\InactiveUsers.csv',
...
Function Find-Accounts {
  ...
  Process {
    Try {
      ...
      Switch ($SearchScope) {
        'All' {
          #$global:Results = Get-ADUser -Searchbase $Searchbase -Filter { (LastLogonDate -lt $InactiveDate -or LastLogonDate -notlike "*") -and (Enabled -eq $true) } -Properties LastLogonDate | Select-Object @{ Name="Username"; Expression = {$_.SamAccountName} }, Name, LastLogonDate, DistinguishedName
          $global:Results = Get-ADUser -Searchbase $Searchbase -Filter { (LastLogonDate -lt $InactiveDate -or LastLogonDate -notlike "*")} -Properties LastLogonDate | Select-Object @{ Name="Username"; Expression = {$_.SamAccountName} }, Name, LastLogonDate, DistinguishedName
        }

        'OnlyInactiveUsers' {
          #$global:Results = Get-ADUser -Searchbase $Searchbase -Filter { LastLogonDate -lt $InactiveDate -and Enabled -eq $true -and SamAccountName -notlike $ServiceAccountIdentifier } -Properties LastLogonDate | Select-Object @{ Name="Username"; Expression = {$_.SamAccountName} }, Name, LastLogonDate, DistinguishedName
          $global:Results = Get-ADUser -Searchbase $Searchbase -Filter { LastLogonDate -lt $InactiveDate -and SamAccountName -notlike $ServiceAccountIdentifier } -Properties LastLogonDate | Select-Object @{ Name="Username"; Expression = {$_.SamAccountName} }, Name, LastLogonDate, DistinguishedName
        }

        'OnlyServiceAccounts' {
          #$global:Results = Get-ADUser -Searchbase $Searchbase -Filter { LastLogonDate -lt $InactiveDate -and Enabled -eq $true -and SamAccountName -like $ServiceAccountIdentifier } -Properties LastLogonDate | Select-Object @{ Name="Username"; Expression = {$_.SamAccountName} }, Name, LastLogonDate, DistinguishedName
          $global:Results = Get-ADUser -Searchbase $Searchbase -Filter { LastLogonDate -lt $InactiveDate -and SamAccountName -like $ServiceAccountIdentifier } -Properties LastLogonDate | Select-Object @{ Name="Username"; Expression = {$_.SamAccountName} }, Name, LastLogonDate, DistinguishedName
        }

        'OnlyNeverLoggedOn' {
          #$global:Results = Get-ADUser -Searchbase $Searchbase -Filter { LastLogonDate -notlike "*" -and Enabled -eq $true } -Properties LastLogonDate | Select-Object @{ Name="Username"; Expression = {$_.SamAccountName} }, Name, LastLogonDate, DistinguishedName
          $global:Results = Get-ADUser -Searchbase $Searchbase -Filter { LastLogonDate -notlike "*" } -Properties LastLogonDate | Select-Object @{ Name="Username"; Expression = {$_.SamAccountName} }, Name, LastLogonDate, DistinguishedName
        }

        'AllExceptServiceAccounts' {
          #$global:Results = Get-ADUser -Searchbase $Searchbase -Filter { LastLogonDate -lt $InactiveDate -and Enabled -eq $true -and SamAccountName -notlike $ServiceAccountIdentifier -or LastLogonDate -notlike "*" } -Properties LastLogonDate | Select-Object @{ Name="Username"; Expression = {$_.SamAccountName} }, Name, LastLogonDate, DistinguishedName
          $global:Results = Get-ADUser -Searchbase $Searchbase -Filter { LastLogonDate -lt $InactiveDate -and SamAccountName -notlike $ServiceAccountIdentifier -or LastLogonDate -notlike "*" } -Properties LastLogonDate | Select-Object @{ Name="Username"; Expression = {$_.SamAccountName} }, Name, LastLogonDate, DistinguishedName
        }

        'AllExceptNeverLoggedOn' {
          #$global:Results = Get-ADUser -Searchbase $Searchbase -Filter { LastLogonDate -lt $InactiveDate -and Enabled -eq $true } -Properties LastLogonDate | Select-Object @{ Name="Username"; Expression = {$_.SamAccountName} }, Name, LastLogonDate, DistinguishedName
          $global:Results = Get-ADUser -Searchbase $Searchbase -Filter { LastLogonDate -lt $InactiveDate } -Properties LastLogonDate | Select-Object @{ Name="Username"; Expression = {$_.SamAccountName} }, Name, LastLogonDate, DistinguishedName
        }
  ...
  Process {
    Try {
      ForEach ($Item in $global:Results){
        ...
        Move-ADObject -Identity $Item.DistinguishedName -TargetPath "OU=Users,OU=Disabled,DC=Pomeroy,DC=msft"
...
Function Delete-Accounts {
  ...
  Process {
    Try {
      ForEach ($Item in $global:Results){
        Get-ADUser -Identity $Item.DistinguishedName | Remove-ADUser -Confirm:$false
