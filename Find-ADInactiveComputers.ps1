...
Param (
  ...
  [Parameter(Mandatory = $false)][int]$DaysInactive = 60,
  [Parameter(Mandatory = $false)][string]$ReportFilePath = 'C:\InactiveReports\Inactivecomputers.csv',
  ...
  Process {
    Try {
      ForEach ($Item in $global:Results){
        ...
        Move-ADObject -Identity $Item.DistinguishedName -TargetPath "OU=Computers,OU=Disabled,DC=Pomeroy,DC=msft"
